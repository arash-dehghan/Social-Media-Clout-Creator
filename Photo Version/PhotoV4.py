...
#note: You need to specify the path to your own chromedriver.exe in line 237
'------------------------------------------------------------------------'
...
def InstagramPoster(photo,description,credit):
    ...
    try:
        mycaption = '"' + description + '"' + " (Created By: u/" + credit + ")\n\n.\n.\n.\n.\n.\n.\n"+insta_hashtags
        try:
            im = Image.open("picture_to_post.jpg")
            rgb_im = im.convert('RGB')
            rgb_im.save('picture_to_post.jpg')
            image,size = prepare_image('picture_to_post.jpg')
            resize_image('picture_to_post.jpg','a1_photo_to_post.jpg',size)

            PostToTheGram(mycaption)
...
def PostToTheGram(caption):
    ...
    driver = webdriver.Chrome(options = chrome_options, executable_path=r'/path/to/your/chromedriver.exe')
    ...
    field.send_keys(instagram_username)
    field = driver.find_element_by_css_selector("input[type='password']")
    field.send_keys(instagram_password)
    time.sleep(2)
    ...
    time.sleep(15)
    button = driver.find_elements_by_xpath('//*[@id="react-root"]/section/div[2]/div[2]/div/div/div/button[1]/span')[0]
    button.click()
